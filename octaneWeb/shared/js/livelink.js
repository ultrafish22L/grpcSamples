...
class LiveLinkClient extends SimpleEventEmitter {
    ...
    async getRenderImage() {
        try {
            // First, restart rendering to ensure we have fresh data
            console.log('🔄 Restarting rendering...');
            const restartResult = await this.makeGrpcCall('ApiRenderEngineService', 'restartRendering', {});
            
            if (restartResult && restartResult.success) {
                console.log('✅ Rendering restarted successfully');
            } else {
                console.warn('⚠️ Failed to restart rendering, continuing anyway:', restartResult);
            }
            ...
            console.log('🔍 Checking if image is ready...');
            const imageReadyResult = await this.makeGrpcCall('ApiRenderEngineService', 'isImageReady', {});
            
            if (imageReadyResult && imageReadyResult.success) {
                console.log('📊 Image ready status:', imageReadyResult.data);
            }
            
            // Set up the async tonemap parameters
            console.log('🔧 Setting up async tonemap parameters...');
            const setupResult = await this.makeGrpcCall('ApiRenderEngineService', 'setAsyncTonemapParams1', {
                bufferType: 0, // TONEMAP_BUFFER_TYPE_LDR
                cryptomatteFalseColor: false,
                colorSpace: 1, // NAMED_COLOR_SPACE_SRGB
                premultipliedAlphaType: 0 // PREMULTIPLIED_ALPHA_TYPE_LINEARIZED
            });
            
            if (setupResult && setupResult.success) {
                console.log('✅ Async tonemap parameters set successfully');
            } else {
                console.warn('⚠️ Failed to set async tonemap parameters, continuing anyway:', setupResult);
            }
            
            // Set the render passes for tonemapping
            console.log('🎯 Setting async tonemap render passes...');
            const passesResult = await this.makeGrpcCall('ApiRenderEngineService', 'setAsyncTonemapRenderPasses', {
                tonemapPasses: {
                    data: [0] // RENDER_PASS_BEAUTY
                }
            });
            
            if (passesResult && passesResult.success) {
                console.log('✅ Async tonemap render passes set successfully');
            } else {
                console.warn('⚠️ Failed to set async tonemap render passes, continuing anyway:', passesResult);
            }
            
            // Trigger async tonemap
            console.log('🎨 Triggering async tonemap...');
            const triggerResult = await this.makeGrpcCall('ApiRenderEngineService', 'triggerAsyncTonemap', {});
            
            if (triggerResult && triggerResult.success) {
                console.log('✅ Async tonemap triggered successfully');
            } else {
                console.warn('⚠️ Failed to trigger async tonemap, continuing anyway:', triggerResult);
            }
            
            // Wait a moment for tonemap to complete
            await new Promise(resolve => setTimeout(resolve, 500));
            
            // Now try to grab the render result
            console.log('📸 Grabbing render result...');
            const grabResult = await this.makeGrpcCall('ApiRenderEngineService', 'grabRenderResult', {});
            
            if (grabResult && grabResult.success) {
                console.log('✅ Grabbed render result:', grabResult);
                
                // Check if we have render images
                if (grabResult.data && grabResult.data.renderImages && grabResult.data.renderImages.data && grabResult.data.renderImages.data.length > 0) {
                    console.log('📸 Found render images:', grabResult.data.renderImages);
                    return grabResult.data;
                } else {
                    console.warn('⚠️ Grab successful but no render images found. Trying synchronous tonemap...');
                    
                    // Fallback to synchronous tonemap
                    const tonemapResult = await this.makeGrpcCall('ApiRenderEngineService', 'synchronousTonemap1', {
                        passes: {
                            data: [0] // RENDER_PASS_BEAUTY
                        },
                        passesLength: 1,
                    ...
                    if (tonemapResult && tonemapResult.success) {
                        console.log('📸 Synchronous tonemap result:', tonemapResult);
                        return tonemapResult.data;
                    } else {
                        console.error('❌ Failed to generate synchronous tonemap:', tonemapResult);
                        return null;
                    }
                }
            } else {
                console.error('❌ Failed to grab render result:', grabResult);
                return null;
            }
            
        } catch (error) {
            console.error('❌ Error in getRenderImage:', error);
            return null;
        }
    }
