...
Camera camera;
MouseState mouse;
GrpcCameraSync grpcSync;
ModernRendering::ModernRenderer renderer;

// Modern rendering using shared header
...
void cursorPosCallback(GLFWwindow* window, double xpos, double ypos) {
    ...
    if (mouse.dragging) {
        ...
        grpcSync.updateCamera(position, camera.center, glm::vec3(0.0f, 1.0f, 0.0f));
...
int main() {
    ...
    if (!grpcSync.connectToServer("/tmp/octane_server.sock", "/tmp/grpc_client.sock")) {
        std::cerr << "Warning: Failed to connect to Octane server" << std::endl;
    }
    
    if (!grpcSync.initialize()) {
    ...
    grpcSync.updateCamera(initialPosition, camera.center, glm::vec3(0.0f, 1.0f, 0.0f));
