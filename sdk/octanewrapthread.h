...
#ifndef _OCTANE_WRAP_THREAD_H_
#define _OCTANE_WRAP_THREAD_H_  1
...
/// You can use this safety check in your code to make sure that the code is executing in the
/// message thread. Very useful for debugging.
#define OCTANE_API_ASSERT_MESSAGE_THREAD                            \
    OCTANE_API_ASSERT(OctaneWrap::Thread::isThisTheMessageThread(), \
                     "this is not the message thread");
...
//--------------------------------------------------------------------------------------------------
/// Wraps ApiThread. You should derive from this class to create your own thread.
class Thread
{
    OCTANEAPI_NO_COPY(Thread);

public:    

    /// Creates a new thread with the passed in name.
    Thread(
    ...
    /// Your code should implement this function. This function is called in a separate thread
    /// after startThread() is called.
    virtual void run() =0;
    ...
    /// Calls the passed in function in the main thread (also referred to as the message thread).
    ...
    void* callFunctionOnMessageThread(   
...
private:
    ...
    // wrapped thread 
    Octane::ApiThread *mApiThread;

    // internal C-style callback
    static void internalThreadRunFunction(
        void *privateData);
};
...
#endif // #ifndef _OCTANE_WRAP_THREAD_H_
