...
#ifndef _API_BINARY_FILE_H_
#define _API_BINARY_FILE_H_ 1
...
class ApiBinaryGroup;
class ApiBinaryTable;

//--------------------------------------------------------------------------------------------------
/// Stores a sorted group of elements with up to one entry per ID. This class acts more like a 
/// handle, multiple objects can refer to the same binary table internally.
class OCTANEAPI_DECL ApiBinaryTable
{
    OCTANEAPI_NO_COPY(ApiBinaryTable);

public:

    /// Create a new binary table. Don't use delete but call destroy() to cleanup.
    static ApiBinaryTable* create();
    ...
    bool equals(
        const ApiBinaryTable &other) const;
    ...
    bool get(const int32_t id, const char *&value) const;

    /// Cleans up a string returned with get.
    void cleanupString(
        const char *str);

    /// Values should be cleaned up via destroy().
    bool getGroup(const int32_t id, ApiBinaryGroup *&group) const;
    bool getTable(const int32_t id, ApiBinaryTable *&table) const;
    ...
    void set(const int32_t id,  const char*            value);

    void setGroup(const int32_t id, const ApiBinaryGroup &group);
    void setTable(const int32_t id, const ApiBinaryTable &table);
...
//--------------------------------------------------------------------------------------------------
/// Stores a group of elements. Multiple entries with the same ID are allowed.
class OCTANEAPI_DECL ApiBinaryGroup
{
    OCTANEAPI_NO_COPY(ApiBinaryGroup);

public:

    /// Creates a new binary group.
    static ApiBinaryGroup* create();
    ...
    bool equals(
        const ApiBinaryGroup &other) const;
    ...
    /// Returned string should be deleted via cleanupString().
    bool get(const size_t ix, const char *&value) const;
    ...
    /// Values should be cleaned up via destroy().
    bool getGroup(const size_t ix, ApiBinaryGroup *&group) const;
    bool getTable(const size_t ix, ApiBinaryTable *&table) const;
    ...
    void addGroup(const int32_t id, const ApiBinaryGroup &group);
    void addTable(const int32_t id, const ApiBinaryTable &table);
...
#endif // #ifndef _API_BINARY_FILE_H_
