...
void RunGrpcServer(MainComponent * pMainWnd)
{
    std::string server_address = gClientURL;// "E:/__wip/grpc_api_2026_1_master/grpc-api/client/socket.tmp";
    std::shared_ptr<grpc::ServerCredentials> creds;
    creds = grpc::InsecureServerCredentials();

    mBuilder.AddListeningPort(server_address, creds);
    ...
    void *                                    userData)
{
    ...
    for (size_t i = 0; i < renderImages.mSize; ++i)
    {
        ...
        std::string imagePath = "E:\\__wip\\__test.bmp";
        //saveAsBMP(imagePath, pixelData, renderImage.mSize.x, renderImage.mSize.y);
        if (pixelData)
        {
            delete[] pixelData;
        }
         ...
         char * argv[])
{
    ...
    std::string path = "E:\\__wip\\__grpcdebuglog_rendertest.txt";
    try
    {
    ...
    ApiRenderEngineProxy::restartRendering();

    //ApiArray<Octane::ApiRenderImage> renderImages;
    //if (!ApiRenderEngineProxy::grabRenderResult(renderImages))
    //{
    //    return false;
    //}
    ...
    const StreamCallbackRequest & request)
{
    ...
    {
        case StreamCallbackRequest::kNewStatistics:
        {
            std::cout << "[Client] Received callback of type: kNewStatistics \n";

            CallbackAck ackResponse;
            auto reqData = request.newstatistics();
            grpc::Status st = callbackHandler.OnNewStatistics(
                nullptr,
                &reqData,
                &ackResponse); 
            break;
        ...
        case octaneapi::StreamCallbackRequest::kRenderFailure:
        {
            ...
            grpc::Status st = callbackHandler.OnRenderFailure(
                nullptr,
                &reqData,
                &ackResponse);
        ...
        case octaneapi::StreamCallbackRequest::kProjectManagerChanged:
        {
            ...
            CallbackAck ackResponse;
            octaneapi::ObserverRequest req;
            auto reqData = request.projectmanagerchanged();
            req.set_callback_id(reqData.callback_id());
            req.set_user_data(reqData.user_data());
            req.set_callback_source(reqData.callback_source());
            grpc::Status st = callbackHandler.ProjectManagerObserver(
                nullptr,
                &req,
                &ackResponse);
            break;
        }
        case StreamCallbackRequest::kNewImage:
        {
            std::cout << "[Client] Received callback of type: kNewImage \n";

            const OnNewImageRequest & img = request.newimage(); //  use correct field name
            CallbackAck ackResponse;
            grpc::Status st = callbackHandler.OnNewImage(
                nullptr,
                &img,
                &ackResponse);
            break;
